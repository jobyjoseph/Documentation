import SEO from "../../components/seo.js";
import Layout from "../../components/layout.js";
import JSLayout from "../../components/masters/javascript-layout.js";
import Image from "../../components/utils/image/image.js";
import imgDoneWithItAccountScreen from "../../images/articles/2020/donewithit-account-screen.jpg";
import imgDoneWithItListingsScreen from "../../images/articles/2020/donewithit-listings-screen.jpg";
import imgDoneWithItCustomPicker from "../../images/articles/2020/donewithit-custom-picker.jpg";
import imgDoneWithItListingEditScreen from "../../images/articles/2020/donewithit-listingeditscreen.jpg";


<Layout>
<SEO title="Soku - The Guessing Game - Part 2" />
<JSLayout>

# Soku - The Guessing Game - Part 2

Part 2 contains the tasks did during the month of August 2020.

### Day 14 - Aug 1, 2020

Learned:

- Create a reusable Icon component.
- Used **Pull to Refresh** feature of FlatList

Total time: **9.5 hours**

### Day 15 - Aug 2, 2020

There are lot of work from my full time job. But, I am somehow finding 30 minutes time just for continuation. I started building the tutorial app's account screen. It is basically using the `ListItem` component I created earlier.


Total time: **10 hours**

### Day 16 - Aug 3, 2020

The account screen of _DoneWithIt_ app is completed. I am now comfortable in using `FlatList` component.

<Image src={imgDoneWithItAccountScreen} style={{maxWidth: "300px", display: "block", margin: "auto"}} alt="Donewithit Account screen" />


Total time: **10 hours**

### Day 17 - Aug 4, 2020

Today, I worked on the Listings Screen of _DoneWithIt_ app. I used the `<Card />` component created earlier. All the data is hard coded. I will be learning API requests in coming lessons.

<Image src={imgDoneWithItListingsScreen} style={{maxWidth: "300px", display: "block", margin: "auto"}} alt="Donewithit listings screen" />


Total time: **10.5 hours**

### Day 18 - Aug 5, 2020

A new section about input controls started in the course. Today, I learned how to use text input. I then created a custom text input component which can also show an icon on the left.

Total time: **11 hours**

### Day 19 - Aug 6, 2020

Today I worked on rest of the custom text input component. Then I learned how `Switch` control works.

Total time: **11.5 hours**

### Day 20 - Aug 7, 2020

I was extremely tired with office work. I opened the React native course. Slept in my chair for few hours and then went to bed.

### Day 21 - Aug 8, 2020

I learned about `Picker` component. I have started building a custom component.

Total time: **12 hours**

### Day 22 - Aug 9, 2020

The custom picker will show a modal window on clicking the picker. Need to complete more.

Total time: **12.5 hours**

### Day 23 - 25 - Aug 10 - 12, 2020

Day and night I am with Microsoft Sharepoint. This is part of my full time job. I take this project late at night and trying to push one line at a time. Like that I completed the _Input Components_ section in Mosh's course. Here is the output:

<Image src={imgDoneWithItCustomPicker} style={{maxWidth: "300px", display: "block", margin: "auto"}} alt="Donewithit custom picker" />

Total time: **13.5 hours**

Next moving on to _Forms_ in React Native.

### Week 5, Aug 16 - 22, 2020

Learned
- to create forms using _Formik_
- to valide _Formik_ forms using _Yup_
- Code refactoring

### Week 6, Aug 23 - 29, 2020

I could not work on the app on most of the days. The course was mainly to create _ListingEditScreen_ by using created components.

The screen looks like this:
<Image src={imgDoneWithItListingEditScreen} style={{maxWidth: "300px", display: "block", margin: "auto"}} alt="Donewithit Listing Edit Screen" />

</JSLayout>
</Layout>
