...
If any of the operator is not a number, then both the operands are converted to string and then **concatenated**.
...
What if one operand cannot be converted to a string? Take following example:

```javascript
console.log(4 + null);
```

In the above case `null` cannot be converted to a string like `"null"`. So JavaScript tries to perform a numeric addition and results in `NaN`.
