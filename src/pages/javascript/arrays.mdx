...
As discussed earlier, `Array.of()` creates and returns a new array from the arguments passed ot the method.


## Array.prototype <a name="array-prototype" />

The `Array()` constructor function has got a property called `prototype`. `Array.prototype` stores an object value. Any properties and methods of this `prototype` object is inherited by all instances of `Array()`. Example, if `Array.prototype` has a method called `boom()`, then `[2, 4, 7].boom()` is a valid method call.
