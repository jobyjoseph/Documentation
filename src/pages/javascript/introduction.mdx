...
When Brendan Eich developed JavaScript, it was initially called _Mocha_. Later renamed to _LiveScript_ and then _JavaScript_. Netscape Navigator 2.0 was released with JavaScript. Later Microsoft

## JavaScript Engine

We all know that JavaScript is mainly used in the context of web. If a browser can recognize and execute JavaScript, it means there is a __JavaScript Engine__ inside that browser.

In case of a programming language like Java, we know that the Java compiler is written and maintained by Oracle. For the same reason, Java developers can blindly use a feature available in a certain version of Java. But in case of JavaScript, the engine used by different browsers are created by the browser owners. So, there can be some features that work only in certain browsers. That being said, all major browsers today provide good support.
